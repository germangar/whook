...
It will do everything it can to fullfill orders, including resending the order until it times out, reducing the quantity or the order when the balance is not enough and dividing the order in two at reversing positions when not enough balance for doing it at once.
